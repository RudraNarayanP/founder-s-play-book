...
| RD-077 | Amazon | `context_appendices.md` | **The instruction-layer retraction residue is now registered as a conflict (U.168)**: `$871,000` and `2,613,000` still appear **twice each** in the Stage-1 appendices, with **zero hits across the 93 filing texts** | RD-059's class, one level deeper — the corpus was swept, the deliverable was not | `s3_p4` re-check | OPEN — one repair pass, marker-only |
