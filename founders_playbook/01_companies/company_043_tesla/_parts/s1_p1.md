...
**Scope of this pass, stated so nothing here reads as a null.** Part 1 was written **against bytes
already on disk**, using the 0-call default web budget, and it read **25 of the 76 held SEC documents**
(the S-1 lineage's six principal documents, four exhibits by title, the FY2010 10-K, the 2011 DEF 14A,
the XBRL series and the two index/manifest files). **No family outside (a) filings was attempted by
this pass, and none of the routes below returned nothing — they were not run.** Per §14.6 an untried
family is not a null, and per RD-103 a broken route reported as a null has repeatedly misled this
project.

**Families not reached by this pass** (all still open; the probe's and regrade's statuses are carried
forward unchanged, and none is *answered*):

- **NOT TRIED by me — (b) web archives.** Zero CDX or snapshot calls made. The probe's blocked routes
  (U-1, U-2) stand: three pre-incorporation `tesla.com` captures are index rows only, **no page byte
  has ever been held by any pass**, and the August 2009 joint statement remains the highest-value
  unwritten object in this company.
- **NOT TRIED by me — (c) periodicals and (d) corporate print.** No IA/HathiTrust/Chronicling America
  query made; `tesla-logo` and `teslaroadster0000maur` are still **identified and never opened**
  (U-5). The **per-item page-text layer has never been searched at all** for this company, which is the
  only route that could produce 2004–2006 in-window text.
- **NOT TRIED by either pass — (e) documentary / auction / museum sale records.** Still no scripted
  route (§Family e, probe). This remains the family most likely to hold something that is neither the
  company's account nor a founder's memory.
- **NOT TRIED by me — (f) legal registers.** No CourtListener or state-docket call. The probe's scope
  finding stands: its federal-only answer is **not** an absence of records, because the California
  Superior Court registry that heard the dispute was never touched.
- **NEW-2, not tried by either pass — USPTO patent records.** An independent, dated, non-corporate
  registry of named inventors and assignees 2004–2008. Nobody has queried it. It is the only family
  that could bear on who did technical work inside the entity **without being anyone's memory of it**,
  and it is therefore the highest-yield untried route for the founder-credit question specifically.
- **NEW-1, not tried by either pass — the SEC staff correspondence in this very filing set.**
  `sources/_index/submissions.csv` lists **UPLOAD 2010-02-25** and **CORRESP 2010-06-08** (plus
  `CERTNAS`, `EFFECT`), and **neither pass fetched them**. A staff comment letter and its reply inside
  the window in which the founder adjective appeared and the March amendment did not carry it are the
  most probative unheld documents this project knows the accession numbers of. Route:
  `sec_intake.py auto 1318605 --company-dir founders_playbook/01_companies/company_043_tesla`
  widened, or `grab` on those accessions.
- **NEW-3 — the 12 REGDEX `.paper` rows (2005-02-17 → 2009-01-12) and Form D 2009-04-09: never
  fetched by any pass.** RD-112 established these rows exist and carry no company narrative, so the
  expected yield is low; naming them anyway, because `auto` printed **0 UNANSWERED while leaving 269
  in-window index rows at 28 fetched accessions** — which is RD-112's silent-drop defect, and "0
  UNANSWERED" is not coverage.
- **NEW-4 — 51 of the 76 held documents unread**, including all 8-Ks, all SC 13G/As (which may date the
  first post-IPO statements of Musk's holding), `dex31.htm`/`dex231.htm` certifications, the
  2011-05-25 S-1, and the two submission-text files of 4.6–4.9 MB.
- **NEW-5 — Delaware Division of Corporations** entity file for the 2003-07-01 charter: never queried
  by any pass. This is the single document that would move §P1-01's date from Medium to High.

**Explicit non-claims of this part.** It does **not** claim that no independent record of the founding
exists; that the founder question is resolved either way; that the corporate dating is wrong; that any
named person is or is not a founder; or that the routes above would fail. It claims that on
2026-09-26, from bytes in `sources/`, Tesla's Stage 1 is documented by **one registrant voice in two
datable layers, now bracketed on both sides by held amendments, plus three contemporaneous
instruments** — and that the enlargement of the corpus from 4 documents to 76 **raised the precision of
the founder-attribution record without adding a single independent witness to it.**
