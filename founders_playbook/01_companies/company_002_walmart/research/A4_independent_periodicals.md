...
**Range note (added on completion):** the heading names A4-re-01 … A4-re-16 but this block
holds **seven** re-walk rows, R1…R7, because the re-walk produced claims rather than further rows: R1…R7 feed
A4-17 … A4-22, and the seven layers named here are the seven Business Week index volumes A4 pulled and read
whole. Six layers became **seven** when 1963/1965/1967 turned out to be half-year bindings (A4-19, A4-30).
The heading is left as first written, per §14 rule 4.
...
**Amendment on completion (nothing above deleted): the count is now 40 claim records** — A4-01…A4-36, A4-41,
A4-42 as first written, plus **A4-43 and A4-44** from the host-family re-probe filed after the CSV work
(§Host-family re-probe below) — plus R1…R7, and **9 contradictions U-A4/1…U-A4/9** (U-A4/3, U-A4/4 were
drafted in §Contradictions but never reached `conflicts.csv` in the first pass; U-A4/9 is new and is about
this machine, not about Walmart).Mart-named coverage, year by year 1962–1980
...
**Amended 2026-09-25, after the harvester's repair and A4's own re-probe (see §Host-family re-probe below;
the paragraph above is retained as written, per §14 rule 4): the block is no longer uniform.**
HathiTrust answers **HTTP 200** from this machine through `curl` (A4 got 369,430 B for one full-view
search; the harvester's probe logged four further 200s), **Google Books** answers 200 on the
`books/feeds/volumes` endpoint but **not** on the date-honouring `www.googleapis.com/books/v1` route, which
returned 429 for A4 as well, and **Chronicling America is still 403** on every probe round. The correct
state of the first two families is therefore **PARTLY RE-OPENED, TEXT UNREAD**, not UNANSWERED-because-
blocked, and not EMPTY either.
...
## Host-family re-probe, 2026-09-25 (filed after the CSV work; changes route state, not evidence)

**Why this section exists at all.** The first pass closed its budget at 42–46 of 60 requests and left 18
**deliberately unspent** on the stated ground that HathiTrust, Google Books and Chronicling America were
blocked from this machine. A harvester repair landed after that decision was taken and its probe evidence is
on disk at `00_universe/harvest/_probe_fixed_20260925/` (`_MANIFEST.md`, 63 evidence files, and
`_REQUEST_LEDGER.tsv`). A4's own gap list names the same three families as the **only** place the missing
witness plausibly sits (§Data gaps UNTRIED first bullet; §Depth-verdict item 7(i)), so the unspent budget was
spent here and only here. **Requests spent by this pass: 10 of a cap of 10** — 3 Google Books feed GETs
(HTTP 200), 1 Google Books v1 API call (**HTTP 429**, 0 B), 5 HathiTrust attempts that returned **no HTTP
status at all** (`SSLCertVerificationError: certificate has expired` — the same failure class the intake
recorded for `archive.org/download/`, and a property of A4's Python client, **not** of the host), and 1
HathiTrust attempt through `curl` (**HTTP 200, 369,430 B**). Reading the harvester's saved bodies cost 0
requests and is labelled as such below.

**Route findings, stated at the level they were actually obtained.**

1. **The route is open; the witness is not found.** HathiTrust's full-view OCR search for the phrase
   `"Wal-Mart"` restricted to the publication decade 1970–1979 returns **573 records** (result page read once,
   page 1 of 9 titles). **No item text has been read.** A search-hit count is not a document and is not a
   naming.
2. **What the first page actually is:** `Statistical bulletin — Securities and Exchange Commission` (v.32:21
   1973, v.32:27 1973, v.33 nos.18-30 1974, and the 1974 May-Dec / Jul-Dec issues), `USITC publication`
   Nos. 2748 and 3105, and a `Nevada municipal airport ADAP, ALP: environmental impact statement` dated 1974.
   Government serials, i.e. a genuinely **different publisher** from the registrant and dated inside Stage 1
   — which is why they are worth the next pass's requests and why they are **not** worth a verdict change
   today.
3. **A lineage caution that must travel with row 2:** the SEC Statistical bulletin republishes what
   registrants file. An item of that class naming "Wal-Mart Stores Inc." would be an independent **publisher
   on a date**, but it would not be an independent **measurement of the company**, and §3's lineage test
   would still have to be applied to its content before it could be counted as a second lineage for any
   figure.
4. **The harvester's own saved HathiTrust bodies, read locally at zero cost, already bound the 1960s**:
   `"Wal-Mart"` + Bentonville, all years = **11,850 records / 2,638 full view**; restricted to **1960–1969 =
   45 / 4 full view**, and those four are `Army logistician` v.97 and three `Customs bulletin : Treasury
   decisions under customs and other laws` volumes (v.29:23-35, v.29:36-41, v.34:1-13). **None is trade or
   general print about a retailer**, and a hyphenated phrase match in 1960s customs and army-magazine OCR is
   far more likely to be noise than to be the Arkansas chain that had two stores. Recorded as the honest
   reading of a 45-record set, not as a null: the 4 full-view titles have not been opened.
5. **Google Books is only apparently open.** The `feeds/volumes` endpoint answers 200 but **ignores its date
   parameters**: with `as_ylo=1960&as_yhi=1975` the ten volumes returned are dated 2000–2023, and the
   harvester's saved `"Wal-Mart" Bentonville Rogers` set is ten volumes dated **1992–2013**. So the earlier
   "429-blocked" row was wrong in one direction and right in another — the **undateable** endpoint works and
   the **dateable** one does not. An in-window Google Books test has therefore still never been run: that is
   **UNTRIED (named query: v1 API `dateRestrict=1960-1975` with a key, or the feed plus a per-volume
   `bibinfo` date check)**, not EMPTY.
6. **The one thing the GB feed did add, free:** its `"National Retail Merchants Association"` query returned
   `The Buyer's Manual — National Retail Merchants Association` (1957) — the same manual family already seen
   on Internet Archive (A4-41) — and **still no membership roster**, which is the document class the whole
   pre-1972 question turns on.
7. **Chronicling America remains 403** in all three probe rounds, including the `wwwloc` declared-UA and
   browser-UA variants; the Wayback CDX route answers (probe ca10/ca11) and is **UNTRIED** for Walmart
   periodical content.

A4-43 Claim: **HathiTrust is reachable from this machine and holds a 573-record full-view candidate set for
the phrase "Wal-Mart" in 1970–1979, so the sentence "the missing witness cannot be obtained from any host
reachable from this machine" is no longer supportable — and no such witness has yet been obtained either.**
Date: probe run 2026-09-25 — Source: A4 curl GET of the babel `ls` search URL (HTTP 200, 369,430 B) plus the
harvester's four saved 200 bodies — Source date: 2026-09-25 — URL: as S0137 — Archived: response bodies in OS
scratch outside the repository; result page deliberately not written into the repo — Tier: n/a (route
evidence) — Class: FACT (as to reachability and hit counts) / **UNKNOWN (as to whether any hit names the
company)** — Passage: NO_VERBATIM_PASSAGE_RECORDED — Conf: High (route), UNKNOWN (content) — Corroboration:
A4's own 200 + the harvester's 4 × 200 on the same host, one machine, one day — Conflicts: **U-A4/9**.
**This record exists to stop a future reader mistaking a re-opened door for evidence walked through it.**

A4-44 Claim: **The next named-witness attempt is now a specific, cheap, ordered task rather than an open
question**: (i) HathiTrust full-view search `"Wal-Mart Stores"` AND Arkansas, decade facets 1970–1979 and
1960–1969 separately, then open the hit text and read it — 2 searches + 1 view per candidate; (ii) HathiTrust
search for the roster genre the corpus lacks (`"National Retail Merchants Association"` + `membership` /
`directory of members`, and `"Chain Store Age"` directory volumes, 1955–1975); (iii) the SEC Statistical
bulletin's listed-companies pages for "Wal-Mart Stores Inc.", counted as an independent publisher only after
§3's lineage test; (iv) open the four 1960–1969 full-view titles and confirm the noise reading. Client note:
use `curl`, not the urllib bundle, until the CA store is fixed. Date: 2026-09-25 — Source: A4-43 plus
§Data gaps — Class: INFERENCE (from the observed composition of the result page; mechanism stated: page 1 of
a relevance-sorted union-catalogue search is government serials, so the productive queries are the narrow
company-name and roster ones, not the broad phrase ones) — Conf: Medium — Corroboration: none — Conflicts:
none. **Registered as the follow-up task for the merge, at 10 requests spent and the cap reached: this pass
may not spend more.**
...
8. **Effect of the host-family re-probe on this verdict (A4-43, A4-44, U-A4/9): none — and the reason is the
   point.** The re-probe re-opened a route; it did not produce a document. `wal-mart` is still **0** in every
   item this dossier has actually read, and the 573 HathiTrust full-view candidates for 1970–1979 have not
   been opened. **The independent-lineage count for the company's Stage-1 record therefore remains 1.** What
   changed is the *prospect* of moving it and the *status of the UNANSWERED rows*: item 7(i) can no longer
   read “blocked”, it must read “reachable, unsearched at text level”. The reverse error must also be
   refused: a search that returns hundreds of hits is not a witness, and the first page of those hits is SEC
   and USITC serials whose Wal-Mart content, if it exists, republishes the registrant's own filings — an
   independent **publisher**, not an independent **measurement** (§3). Until an item is opened and its
   sentences read, the honest count of independent contemporaneous witnesses naming this company is
   **zero**, and the depth verdict of this dossier is **PROVISIONAL, unchanged, and provisional because of a
   missing document rather than a missing route**.

9. **Interim instruction to the next pass, so the re-opened route is not mistaken for a closed question:**
   spend its requests in the order at A4-44 (narrow company-name + Arkansas, then the roster genre, then the
   SEC-bulletin pages, then the four 1960s full-view titles), read the item text, and only then re-open this
   verdict. A hit list is index-level evidence and stays index-level (§5 as applied here).
...
**COR-A4-10 — to `MASTER_RESEARCH_LOG.md`, to `B_periodical_retest.md`, and to the fleet's periodical
harvest brief.** The host-reachability state recorded in those files (and in A4's own §Working method and
§Data gaps, which are **retained, not edited**) is superseded as of 2026-09-25T13:51Z:
**HathiTrust answers HTTP 200 from this machine through `curl`** — Python's `urllib` on the same machine and
the same URL fails with `SSLCertVerificationError: certificate has expired`, so the block that three passes
recorded was a **client CA-store artefact, not a host policy**; **Google Books' `books/feeds/volumes`
answers 200 but silently ignores `as_ylo`/`as_yhi`** (a ten-of-ten return of 2000–2023 volumes against a
requested 1960–1975 window is the proof), which means **any earlier “date-restricted Google Books EMPTY”
in this project is a false test and must be re-examined**; the date-honouring v1 API still returns **429**
keyless; **Chronicling America is still 403** on every probe round and stays UNANSWERED. **Action:** merge
(1) updates the fleet reachability table, (2) flags every prior Google Books date-filtered null as suspect
rather than as evidence of absence, (3) re-points Walmart's named-witness brief at the four ordered queries
in A4-44, and (4) records that A4 spent 10 requests on this and reached its cap, so the next pass starts at
zero and not at a re-probe of the same doors.
...
Walmart,stage1,1970-1979,"ROUTE-STATE (not a company metric) — HathiTrust full-view records whose OCR text matches the phrase 'Wal-Mart' with the publication-decade facet 1970-1979",573,records-matching-search,"S0137 (A4-43)",2026-09-25,FACT,High,,"Reachability and candidate-set count only: NO item text has been read, so no naming is established and the independent-lineage count is UNCHANGED. Page 1 of the result set is U.S. government serials (SEC Statistical bulletin 1973-74, USITC publications Nos. 2748/3105) plus a 1974 Nevada airport environmental statement — which are the kind of hit that must be read before being counted. Recorded because it is the first candidate pool this project has held for the years after the listing."
```
...
Walmart,stage1,1970-1979,"Host-family re-probe: HathiTrust — recorded by every earlier pass as a TLS failure — answers HTTP 200 for a full-view phrase search of 'Wal-Mart' restricted to the 1970s (573 candidate records); Google Books' volumes feed answers 200 but ignores its date parameters and its date-honouring v1 API still returns 429",HathiTrust; Google Books; A4 re-probe pass,United States,S0137 S0138,LEAD,Medium,none,"A research-state event, not a company event, and not a witness: no item text read. It is registered here because it changes what the next pass can reach and therefore changes the status of the UNANSWERED rows that held the depth verdict at PROVISIONAL. Chronicling America is still 403."
```
...
S0137,1,Route re-probe 2026-09-25 (A4-43): HathiTrust's full-view OCR search for the phrase Wal-Mart restricted to the publication decade 1970-1979 returns 573 records where every earlier pass of this project recorded a TLS failure,"HathiTrust babel full-view search 'Wal-Mart', decade facet 1970-1979",HathiTrust (depositor-library aggregate),union-catalogue search result set (route probe),secondary,1970/1979,2026-09-25,2026-09-25,https://babel.hathitrust.org/cgi/ls?field1=ocr;q1=%22Wal-Mart%22;a=srchls;lmt=ft;facet=bothPublishDateRange%3A%221970-1979%22,"curl response body in OS scratch outside the repo (369,430 B; HTTP 200); result page deliberately NOT written into the repository",—,LEAD,High,"A different HOST family, not a different lineage of a publisher: the top-ranked matches on page 1 are U.S. government serials (SEC Statistical bulletin 1973-1974; USITC publications) which reprint registrant-supplied data, so even a confirmed naming would have to be met with the §3 lineage test rather than assumed to be independent","NO_VERBATIM_PASSAGE_RECORDED (search-result metadata only: 9 titles and their htid read off the result page: 'Statistical bulletin - Securities and Exchange Commission' 1973/1974 volumes; 'USITC publication' Nos. 2748 and 3105; 'Nevada municipal airport ADAP, ALP: environmental impact statement' 1974)","Requests spent on this re-probe: 10 of a hard cap of 10 — 3 Google Books feeds/volumes GETs (HTTP 200), 1 Google Books v1 API call (HTTP 429 Too Many Requests, 0 B), 5 HathiTrust attempts that returned no HTTP status at all under the urllib client (SSLCertVerificationError: certificate has expired — same failure class the intake recorded for archive.org/download), 1 HathiTrust attempt under curl (HTTP 200, 369,430 B). Text of no matching item has been read: this row is a POINTER, not a witness, and does not move the company lineage count"
S0138,1,"Google Books reachability is asymmetric and its date filter is not: the books/feeds/volumes endpoint answers 200 but IGNORES as_ylo/as_yhi, while the v1 API that does honour dateRestrict still returns 429 keyless — so Google Books cannot yet be tested for an in-window 1962-1975 naming","Google Books volumes feed and v1 API, Wal-Mart and NRMA queries 2026-09-25",Google Books,search API reachability probe,secondary,1962/1975,2026-09-25,2026-09-25,https://books.google.com/books/feeds/volumes?q=%22Wal-Mart%22&as_ylo=1960&as_yhi=1975&maxResults=20 ; https://www.googleapis.com/books/v1/volumes?q=%22Wal-Mart%22&dateRestrict=1960-1975&maxResults=20,feed bodies read from the harvester's own saved probe evidence in 00_universe/harvest/_probe_fixed_20260925 (0 requests); A4's three feed GETs held in OS scratch,—,UNANSWERED (route partly open),High,"reachability evidence about THIS machine, not about Walmart, and never to be recorded as a null finding",NO_VERBATIM_PASSAGE_RECORDED,"Proof that the feed ignored the window: all 10 volumes it returned are dated 2000-2023 against a requested 1960-1975 window, and the probe's own 25,023 B 'Wal-Mart' Bentonville Rogers result set is 10 volumes dated 1992-2013 — nothing in-window. The NRMA-titled feed query did reach 1955-1975 print and returned 'The Buyer's Manual - National Retail Merchants Association' (1957), i.e. the manual family already seen on Internet Archive, still no membership roster. Chronological America remains 403 (probe: 6 x 403, three rounds)"
```
...
Walmart,stage1,U-A4/2,T; A4 §Contradictions,Kresge 1966 cover story at p.26 (Jan.29 1966),sim_business-week_1966_index_0 lines 18506-18507,1966-01-29,Kresge 1966 cover story at p.126,sim_business-week_1966_index_0 lines 8338 and 26577,1966-01-29,the same OCR duplication mechanism; equal printings from the same volume,each reading has one printing and neither is independent,"no page is asserted; the entry is cited by headline, cover status and date only",unresolvable on this route,High that a Kresge cover story existed on that date; UNKNOWN as to its page
Walmart,stage1,U-A4/3,E; H; A4 §Contradictions,The discount sector was in financial distress and consolidating by failure in 1962: Business Week's index carries four failure/bankruptcy/shake-out stories plus an industry-level 'facing a major shakeout' statement inside twelve months,"S0112 (BW 1962 index, p.101 Oct.6, p.83 Oct.27, p.146 Sep.22, p.78 Dec.1)",1962-12-01,"The registrant's own reports describe the 1962 founding as a planned, untroubled move, repeated in consecutive annual reports",WALMART_AR_1972.txt through WALMART_AR_1980.txt (A3 §7.2(c)); A3 origin narrative,1972-03-22,different subjects: A is an industry diagnosis made in real time by a third party; B is a company's later account of its own decision — neither is a factual contradiction about the same variable,"A is contemporaneous and third-party (Tier 2, index-level); B is retrospective and single-lineage (Tier 1 as to the company, none as to the sector)","both are recorded, and the dataset states that the sector condition in which the company was founded is independently evidenced while the founding decision itself is not",whether Wal-Mart's founders read or acted on the shake-out coverage: NO evidence either way and no inference is offered,High that the two claims are of different kinds
Walmart,stage1,U-A4/4,E; P; A4 §Contradictions,A3 §7.2(c) requires an independent lineage to corroborate the registrant's figures — i.e. a non-company publication carrying a Wal-Mart number,A3 §7.2(c) and §7.2(d),1972-03-22,"No periodical reached in A4 carries any Wal-Mart figure at all, so nothing exists to disagree with: 0 press figures across 10 BW index years, 5 Stores annual indexes and 8 Stores monthlies",S0112-S0136; §Independent figures final row,1980-04-01,"this is a conflict of expectation against absence, not of two incompatible values; the register must not read the empty row as a resolved corroboration",high-confidence absence within a stated perimeter; the perimeter excludes three host families that were then unreachable and one (HathiTrust) now re-opened unread,the row is recorded as EMPTY-within-the-corpus and the independent-lineage count stays at 1; no silent substitution of a sector figure for a company figure,"whether any of the 573 HathiTrust full-view 1970s records (S0137) carries a Wal-Mart number: UNKNOWN, no text read",High that the row is empty in the corpus read; UNKNOWN as to the world
...
Walmart,stage1,U-A4/8,T; A4 §Findings,"Small town greets the discounters p.90 Oct.3 1964 is indexed under ALDENS, Inc.",sim_business-week_1964_index_0 offset 665015,1964-10-03,The same headline in the DISCOUNT Houses block continues Gamble-Skogmo is opening franchised ...,sim_business-week_1964_index_0 offset 828735,1964-10-03,Business Week's company alphabet and its subject block attribute the same story to two different Minnesota-based variety and mail-order operators,"same publisher, same volume, both placements genuine",immaterial to the verdict and therefore NOT adjudicated: under either attribution the article is a competitor's small-town discount expansion,which firm the article chiefly concerned; resolvable only from the unreachable article text,High that neither attribution supports a Wal-Mart reading
Walmart,stage1,U-A4/9,A4 §Data gaps / §Depth-verdict; MASTER_RESEARCH_LOG,"A4's own UNANSWERED list and B_periodical_retest.md record HathiTrust as a TLS failure and Google Books as 429, and A4's method section says the 18 unspent requests were withheld because those hosts 'would return the same 403 / TLS failure / 429'","A4 §Working method (written 2026-09-25, before the repair); B_periodical_retest.md",2026-09-25,"The harvester's repair shows both families answering: the probe records 4 x HTTP 200 from babel.hathitrust.org (391,767 / 370,909 / 238,566 / 34,171 B) and repeated 200s from books.google.com feeds, and A4's own re-probe got 200 / 369,430 B through curl",00_universe/harvest/_probe_fixed_20260925/_MANIFEST.md and _REQUEST_LEDGER.tsv; S0137; S0138,2026-09-25,"the environment changed mid-dossier and the client differs: A4's urllib carries a CA bundle that sees an expired certificate on babel.hathitrust.org, while curl on the same machine negotiates the same host at HTTP 200, so 'blocked' was true of one client and not of the machine","the probe evidence is the harvester's own byte-verified log and A4 reproduced one call independently; both legs are route evidence, not company evidence","A4's blocked-host rows are AMENDED not deleted: Chronicling America remains 403 (verified again in the probe, three rounds); Google Books is open only on the undateable feeds endpoint (v1 API 429); HathiTrust is open and is where the named-witness search now has to run","no HathiTrust item text has been read, so whether the re-opened route contains the witness is UNKNOWN; and the GB feeds endpoint's date params are ignored, so its in-window test has never actually been run",High that the route state changed; UNKNOWN as to what it yields
```
...
7. **Quoting repair (this pass; the first pass died here).** The rows as first emitted put **25 of 25
   `sources.csv` rows and 5 of 6 `conflicts.csv` rows** at 19–27 and 16–24 columns against the 18- and
   15-column schema, because comma-bearing fields were left unquoted; `quantitative.csv` and `timeline.csv`
   were already clean. Every row has been re-emitted through a CSV writer and re-validated against
   `company_001_amazon/*.csv`: quantitative **12**, timeline **11**, sources **18**, conflicts **15**
   columns in every row, no newline inside any field, every comma-bearing field quoted, `stage1` as the
   stage value throughout. No field's wording was altered by that repair except the four corrections declared
   at notes 8–10.
8. **Citation slips corrected in the rows (declared, not silent).** `S0127` cited **A4-40** in
   `claim_supported`, a reserved ID carrying no record (§Numbering note): now **A4-23/A4-29**. `S0125` and
   `S0126` cited **A4-24**, which is the June-1951 Arkansas-congressman record and not the *Stores* index
   record: now **A4-18**. A4-37…A4-40 remain reserved and unnumbered.
9. **Two tallies disagree inside this dossier and both are kept rather than reconciled.** §Working method
   says **42 of 60** requests (41 × 200 + 1 timeout, 18 left); §Provenance ledger says **46 of 60** (45 × 200
   + 1 timeout, and “18 left”, which cannot both hold). Bytes are **10,015,606** in the two narrative
   statements and **10,035,277** in the `S0136` notes cell. The ledger itself sits in the first pass's OS
   scratch outside the repository, so neither figure is re-derivable here. **The merge must treat this
   dossier's request and byte totals as a stated range, not as settled counts**, and must not copy either
   into `MASTER_RESEARCH_LOG.md` as exact. A4's own re-probe adds **10 further requests** on top of whichever
   tally is right, all listed in the `S0137` notes cell.
10. **Rows added by this pass.** `sources.csv` **S0137** (HathiTrust full-view search route; 573-record
    candidate set 1970–1979; pointer only, text unread) and **S0138** (Google Books feed/v1 reachability; the
    date-parameter finding). One `quantitative.csv` row carrying the 573 count, metric-labelled **ROUTE-STATE**
    so it can never be read as a company metric. One `timeline.csv` row, `evidence_class=LEAD`, recording a
    research-state event and expressly not a company event. `conflicts.csv` gains **U-A4/3** and **U-A4/4**,
    both drafted in §Contradictions by the first pass and never carried into the register, and **U-A4/9**,
    the dossier-internal conflict between its own blocked-host rows and the re-probe. Register totals for the
    append: quantitative **15**, timeline **24**, sources **27**, conflicts **9** — **75 data rows**.
11. **Empty-cell and `derived_arithmetic` convention, checked against `company_001_amazon`:** the only empty
    cells in the append are `quantitative.csv` `derived_arithmetic` on non-DERIVED rows, which is exactly how
    Amazon's register is built; the single DERIVED row (average sales per NRMA member establishment, 1961)
    carries its arithmetic in full (`19000000000 / 11500 = 1652174`), and it is the only DERIVED row in this
    append because every other figure here is read off print rather than computed.

---

## Completion note

**What the first pass had already done, and did well (unchanged by this pass).** Thirty-eight claim records
(A4-01…A4-36, A4-41, A4-42) plus seven re-walk rows and seven contradictions, built on 22 periodical text
layers read whole with byte-exactness tests; the ten-year Business Week `wal-mart` = 0 result with the three
half-year bindings discovered and closed; eight decoy families located by character offset; the
Aldens/Gamble-Skogmo false positive caught (A4-20); the *Stores* subject-only-index caveat that caps five of
the EMPTY rows at Medium (A4-18); the genre argument (A4-29) and the absent-roster finding (A4-41) that
together explain the pre-1972 silence without pretending to prove it; the §H sector reconstruction; and
nine outbound corrections addressed to named files. None of that was restarted or rewritten.

**What it left undone when it died at the turn limit, and what this pass added.**

1. The `## CSV append rows` section was **structurally broken, not unfinished**: all four row blocks were
   present, but `sources.csv` and `conflicts.csv` had unquoted comma-bearing fields and so failed the schema
   (see Register note 7). Repaired, re-validated, and the two missing conflict rows added.
2. `conflicts.csv` carried U-A4/1, /2, /5, /6, /7, /8 only, although §Contradictions had drafted U-A4/3 and
   U-A4/4 — both now in the register, so the merge sees the sector-versus-origin-narrative conflict and the
   empty press-figure row, which is the verdict-bearing one.
3. Two typos that would have become data: `only only` in U-A4/2's interpretation cell and `Wwhich` opening
   U-A4/8's residual-uncertainty cell. Both cells are now clean text.
4. **The dossier's own host-state claim had gone stale mid-run.** It had told itself the remaining 18
   requests were worthless because HathiTrust and Google Books were blocked, and its gap list named exactly
   those families as the only place the missing witness could sit. This pass spent **10 requests (its cap)**
   testing that: 1 HathiTrust 200 (369,430 B, 573 candidate records for 1970–1979), 1 Google Books v1 429,
   3 Google Books feed 200s that proved the endpoint ignores date parameters, 5 HathiTrust attempts that
   failed on this client's CA bundle, plus zero-cost reads of the harvester's saved probe bodies. New records
   **A4-43**, **A4-44**, new section **§Host-family re-probe**, amended UNANSWERED row, depth-verdict items
   **8** and **9**, and outbound **COR-A4-10** addressed to `MASTER_RESEARCH_LOG.md`,
   `B_periodical_retest.md` and the fleet brief.
5. No empty headings were found; the re-walk heading's range (A4-re-01…16 against seven rows R1…R7) was
   undeclared and is now declared rather than renumbered, and the records tally is amended in place rather
   than replaced.
6. Nothing outside this file was written; the two extract files and every register CSV, `A2`, `A3`, `A`, `B`,
   `MASTER_RESEARCH_LOG.md` and the intake are untouched; nothing was deleted (§14 rule 4).

**Depth verdict, stated last and to be read last.** **No: the independent-lineage count does not move above
1.** Not one document in everything mined by either pass names Wal-Mart, so the financial register remains
`DEPTH-CORE (single-lineage)` and the company-level Stage-1 verdict remains **PROVISIONAL** — while the
market-state layer (§H/§I, 1958–1971) is now genuinely second-lined by Business Week's ten indexed years
and *Stores*' 1958 survey and December-1961 seminar report, and can be rated up on that basis alone. The
specific documents that would move the count, in the order they should now be attacked, are: an
**NRMA/NRDGA membership roster or chain-store directory** for 1955–1975 (the genre that names a small
franchisee, still absent from Internet Archive, still untested on the newly reachable HathiTrust); a
**HathiTrust full-view item whose text prints “Wal-Mart” or “Wal-Mart Stores” in 1962–1975** — 573
candidates exist for 1970–1979 and none has been opened; a **Dun & Bradstreet discount-store count**
(dated by A4-31 to p.79, Nov.9 1963, still UNTRIED); and an **Arkansas, Missouri, Kansas or Oklahoma daily**
— which stays UNANSWERED, because Chronicling America is still 403. This dossier does not round any of that
up, and does not convert a re-opened route into evidence.
