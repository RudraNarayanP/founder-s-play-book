...
Every route this pass did **not** run, with the one thing each needs. A route not attempted is not a null, and
none of the sections above rests on any of them (web budget: **0 calls**; nothing was fetched).

1. **`sec_intake.py auto` / `index` — deliberately NOT re-run.** `auto` writes into `sources/sec/` and
   `sources/_MANIFEST.csv`, and `index` overwrites `sources/_index/submissions.csv`; those are protected
   archive paths this agent does not own (§14 rule 4, and RD-112's live demonstration of an `index` write
   clobbering another company's index). The filings facts this volume needs were therefore **re-measured from
   the stored index and the 16 stored documents on disk**, not re-fetched: 4,525 rows, oldest `filingDate`
   1994-02-14, 0 rows earlier, no `S-1`/`S-1/A`, oldest registration-form row the 1994-10-14 S-3. **Tool defect
   recorded, not worked around silently:** none hit — but see item 12 for two hazards found in the corpus's own
   grep habits.
2. **Family (b) web archives: 0 calls.** CDX carried UNANSWERED from both probes (503 ×2); no re-run. §14 rule
   6 makes its Stage-1 weight low regardless.
3. **Family (e) documentary: 0 calls.** No scripted route exists in `tools/`. This is the only realistic route
   to a partnership instrument, licence paper or a signed 1975 document (Q.3, GAP-K2).
4. **The three named page-numbers for the first public statement** — `MITS Computer Notes` Feb 1976 p.3,
   `People's Computer Company` Mar-Apr 1976 p.24, `Radio Electronics` May 1976 p.14 — never harvested;
   `tools/queries.json` **still has no `microsoft` task block**, so the family-(c) harvester would plan 0
   requests for this company even today. GAP-L1.
5. **~500 unopened items of the numFound-545 hobbyist-serial shelf**, plus `byte-magazine-1977-08` (the APL
   follow-up), `Kilobaud197810`, `kilobaudmagazine-1980-04`, `197811PopularElectronics`, Popular Electronics
   Jul-Dec 1975 and Kilobaud 1975. Q.1/GAP-M1 stay open until these are grepped.
6. **A licence/royalty search inside `197602-modern-data_djvu.txt`.** Only the company-name pattern has ever
   been run against it (0 hits); a royalty or licence-language search has not. Cheap: the bytes are on disk.
7. **`Micro-Soft` as a PRIMARY search term** against 1975-1982 serials — the hyphenated form has only ever been
   met incidentally, and it is the term that would find a 1975 document if one exists.
8. **Copyright Office records for 1976-1978 BASIC registrations** — the route that could settle U.9 (who owned
   Altair BASIC). Never queried, by any pass.
9. **New Mexico and Washington Secretary of State / assumed-name and corporation records** — the route for U.6,
   U.11, U.14 and GAP-K2. 0 calls.
10. **1982-1985 and 1989-1990 periodical harvest**, the 159 BYTE 1986-89 / 57 PC Magazine / 26 InfoWorld items
    the probe inventoried and never mined, the 3,468 unfetched 1994-1999 accessions, and `sec_intake.py facts`
    for XBRL 1994-1999 (it printed a path and wrote 0 bytes on the probe's run — C-3 — so even that null is a
    tool state, not a finding).
11. **Bytes on this shelf that this pass did not open:** the 1981 Adventure brochure beyond the one sentence
    part 1 carried; `microsoft-multi-tool-word-brochure` (1983), `16-Bit_Operating_Systems_A_Whole_New_Ball_Game_1982-06`,
    and the two `internetarchivebooks` 1984 rows, which are `inlibrary`/`printdisabled` and so gated by §4
    before anyone reads them; the one `.html` document under `sources/sec/`; and Homebrew `hcc0205`-`hcc0213`
    whole-issue, whose dates remain UNVERIFIED on their own faces (B1's note) and which no record here depends
    on.
12. **Gate coverage not run, by instruction rather than oversight.** The GATE line for this pass is
    `--checks keys,anchors`; `csv`, `budget`, `corrections` and `quotes` were therefore not exercised here.
    `csv` cannot run because **no register CSV exists yet for this company** (§15.2/merge state), which is the
    expected pre-merge condition RD-117 and RD-118 both recorded. Two further checks this pass could not ask a
    gate to do and did by hand instead: a **whitespace-tolerant** search across filings (a single-line grep
    under-counts the founding sentence 6 against 12 and misses the FY1994 10-K entirely), and a **word-boundary**
    search before naming any acronym counterparty (17 apparent MITS/Altair hits resolved to *limits*, *permits*,
    *transmits*). Both are §15.5-shaped: a detector that cannot catch its own miss reports success on a null.

**Nothing in §K-§U is left unproven by an untried route.** Where a route would change a finding, the finding
says so inside the section it belongs to (U.6, U.9, U.11, U.14, Q.2, Q.6), and the register rows carry
`follow_up_task` on every High-importance gap.
