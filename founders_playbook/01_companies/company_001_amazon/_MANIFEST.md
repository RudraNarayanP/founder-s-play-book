...
| `context_appendices.md` | 10,882 | 72,450 | Environment appendices A–J | 1 | MERGED — **the four sites the 2026-09-24 closure could not reach were reached by the AUDIT-6 repair pass on the same date** *(miscounted as "three" in this cell until 2026-09-25; the AUDIT-7 repair pass re-counted the list it prints and it holds four items, and the work itself was verified site by site — no site was added or removed, only the tally was wrong)*: §I "Capital raised inside Stage 1" (a Value cell running the retracted $871,000 / 2,613,000 / $976,408 composition, now a RETRACTION with the composition at UNKNOWN), **§J** gap row 12 *(labelled "§G" in this cell until 2026-09-25 — the gap table is `## J. Data gaps and contested figures` at `context_appendices.md` l.623, header l.628, the row at l.641; §G is "Institutional support environment" at l.467 and does not contain it)* (same leg, same fix), and §I's headcount row plus §F's T1-line row (both counted the original S-1 as corroborating its own amendments; demoted to method §3's version-evidence wording). Audit trail: `03_quality_control/audit6_repairs.md` IR-04/05/13; the miscount and the mislabel are corrected at `03_quality_control/audit7_repairs.md` row R10. |
...
| `U_CONCORDANCE.md` | 5,143 | 32,280 | **canonical conflict mapping U.1–U.42**, adjudicating three independent registers | the only authority for `Conflicts: U.n` references — **does not yet list U.43**, appended 2026-09-24 by the append-only rule (residual defect); and **its U.17 cell printed the retired COR-03 as governing** until 2026-09-25 — COR-03 is SUPERSEDED BY COR-12 and that instruction is WITHDRAWN, so a **SUPERSESSION NOTICE now heads the file** and the cell carries an in-place marker; `conflicts.csv` U.17 is the authority. Logged at `03_quality_control/audit7_repairs.md` row R7 |
| `NUMBER_DEFECTS.md` | 3,348 | 21,505 | the numbers defect register, rounds 1–2 **+ the AUDIT-7 addendum of 2026-09-25** | row 43 is where the false `+95.2%` was imported from; round-2 footer records propagation; **the AUDIT-7 addendum at its foot withdraws the register's `$871,000` / `$976,408` "corrected value" limbs and marks rows 27–28 in place** — see `audit7_repairs.md` rows R4/R4b |
